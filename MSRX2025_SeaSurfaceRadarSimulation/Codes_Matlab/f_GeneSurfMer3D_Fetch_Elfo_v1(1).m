...
if prec == 1 % Single precision 
    % ZE = zeros( [ n_x n_y ], 'single' ) ; 
    HH_XY = zeros( [ n_x n_y ], 'single' ) ; 
    SP_KXKY = zeros( [ n_x n_y ], 'single' ) ; 
    NOR_XY = zeros( [ n_x n_y ], 'single' ) ; 
    X = zeros( [ 1 n_x ], 'single' ) ; KX = zeros( [ 1 n_x ], 'single' ) ; 
    Y = zeros( [ 1 n_y ], 'single' ) ; KY = zeros( [ 1 n_y ], 'single' ) ; 
elseif prec == 2 % Double precision 
    % ZE = zeros( [ n_x n_y ] ) ;
    % HH_XY = zeros( [ n_x n_y ] ) ; 
    % SP_KXKY = zeros( [ n_x n_y ] ) ; 
    % NOR_XY = zeros( [ n_x n_y ] ) ; 
else
    error('prec should be either 1 (-> single precision) or 2 (-> double precision)') ; 
end
