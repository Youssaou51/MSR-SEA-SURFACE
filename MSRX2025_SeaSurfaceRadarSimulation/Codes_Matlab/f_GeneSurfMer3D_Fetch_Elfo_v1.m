function [ HH_XY , X , Y , SP_KXKY , KX , KY , NOR_XY ] = ...
    f_GeneSurfMer3D_Fetch_Elfo_v1 ( u10 , phiw_d , l_x , l_y , n_x , n_y , sd , prec , fetch_m ) 

%--------------------------------------------------------------------------
% FUNCTION: f_GeneSurfMer3D_Fetch_Elfo_v1
%--------------------------------------------------------------------------
...
% NOR_XY: normalization for the IFFT
%--------------------------------------------------------------------------
% COMMENTS:
...
HH_XY   = zeros([n_x n_y], prec_type);
SP_KXKY = zeros([n_x n_y], prec_type);
NOR_XY  = zeros([n_x n_y], prec_type);
X       = zeros([1 n_x], prec_type);
Y       = zeros([1 n_y], prec_type);
KX      = zeros([1 n_x], prec_type);
KY      = zeros([1 n_y], prec_type);

% HH_XY = ZE ; SP_KXKY = ZE ; NOR_XY = ZE ;
% X = zeros( [ 1 n_x ] ) ; KX = zeros( [ 1 n_x ] ) ;
% Y = zeros( [ 1 n_y ] ) ; KY = zeros( [ 1 n_y ] ) ; 
%
% clear ZE
...
% Calcul (...) du spectre
fprintf('Surface de Mer : calcul du spectre d''Elfouhaily et al. : \n') ;
...
MM_KXKY = elfo_tot( u10 , KK , omg ) ; % Partie isotrope   du spectre
DD_KXKY = elfo_ang( u10 , KK , omg ) ; % Partie anisotrope du spectre
%
% MM_KXKY2 = f_SpectrumSea_Iso_Elfouhaily( u10 , KK , omg , [] ) ;
% [~, DD_KXKY2] = f_SpectrumSea_Ang_Elfouhaily( u10 , phiw_d , KK , omg , [] ) ;
%
MM_KXKY2 = f_SpectrumSea_IsoFetch_Elfouhaily( u10 , KK , fetch_m , [] ) ;
[~, DD_KXKY2] = f_SpectrumSea_AngFetch_Elfouhaily( u10 , phiw_d , KK , fetch_m , [] )  ;
%
[ym_a , im_a] = max( abs(MM_KXKY(:) - MM_KXKY2(:)) ) ;
ys_r = std(MM_KXKY(:) - MM_KXKY2(:) , 1) ;
[ym_r , im_r] = max( 100 * abs((MM_KXKY(:) - MM_KXKY2(:))./MM_KXKY(:)) ) ;
[ ym_a MM_KXKY(im_a) max(MM_KXKY(:)) ys_r ] 
[ ym_r MM_KXKY(im_r) ] 
%
[ym_a , im_a] = max( abs(DD_KXKY(:) - DD_KXKY2(:)) ) ;
ys_r = std(DD_KXKY(:) - DD_KXKY2(:) , 1) ;
[ym_r , im_r] = max( 100 * abs((DD_KXKY(:) - DD_KXKY2(:))./DD_KXKY(:)) ) ;
[ ym_a DD_KXKY(im_a) max(DD_KXKY(:)) ys_r ] 
[ ym_r DD_KXKY(im_r) ] 
%
fprintf('Fin calcul spectre \n') ;
%
...
sh_Elfo2 = sqrt( trapz(KY,trapz(KX, MM_KXKY2 .* (1+COs2Phi.*DD_KXKY2)./(2*pi * KK))) ) ;
Sh___CM_Elfo0_Elfo1_Elfo2 = [ sh_CM sh_Elfo0 sh_Elfo1 sh_Elfo1 ] 
sh_Elfo1 - sh_Elfo2 
...
spx_Elfo2 = sqrt( trapz(KY,trapz(KX,MM_KXKY2 .* (1+COs2Phi.*DD_KXKY2)./(2*pi).*KK.*COsCarrePhi)) ) ; % Voir These Christophe, eq. I.85
Spx___CM_Elfo0_Elfo1_Elfo2 = [ spx_CM_ spx_Elfo0 spx_Elfo1 spx_Elfo2 ] 
...
spy_Elfo2 = sqrt( trapz(KY,trapz(KX,MM_KXKY2 .* (1+COs2Phi.*DD_KXKY2)./(2*pi).*KK.*SInCarrePhi)) ) ; % Voir These Christophe, eq. I.85
Spy___CM_Elfo0_Elfo1_Elfo2 = [ spy_CM_ spy_Elfo0 spy_Elfo1 spy_Elfo2 ] 
%
...
randn('seed',double(sd))     ; BR = randn([n_x n_y], prec_type) ; 
randn('seed',double(sd+100)) ; BI = randn([n_x n_y], prec_type) ; 
% rng(double(sd))     ; BR = randn([n_x n_y]) ; 
% rng(double(sd+100)) ; BI = randn([n_x n_y]) ; 

B0 = BR + 1i * BI ;
clear BR BI
BH = zeros( [ n_x n_y ], prec_type ) ;
...
%
% Partie ISOTROPE du spectre de Elfouhaily (gravite + capilarite)
% 
%   S = elfo_tot( u10 , k , omega )
%
% Entrees :
%
%   u10     : Vitesse à 10 m en m/s
% 	k 	    : Nombre d onde en rad/m
%	Omega   : Inverse de l age de la vague
%
% Les equations sont tirees de l'article :
%
% 	T Elfouhaily B Chapron K Katsaros and D Vandemark,
%   "A unified directional spectrum for long and short wind-driven waves,"
%	JGR, vol. 102(C7), pp. 15781-15796, 1997.
%
%   C. BOURLIER le 22/06/2006.

function S = elfo_tot( U10 , k , omega )
...
% Amplitude term of the gravity : equation (34)

alpha_p = 6e-3 * sqrt( omega ) ;

% Amplitude term of the capillarity : equation (44)

C10 = ( 0.8 + 0.065 * U10 ) * 1e-3 ;
V_f = sqrt( C10 ) * U10 ;

if V_f <= cm
	%alpha_m = 1e-2 * ( 1 + abs(log(V_f/cm)) ) ;
	alpha_m = 1e-2 * ( 1 + log(V_f/cm) ) ;
else
	alpha_m = 1e-2 * ( 1 + 3 * log(V_f/cm) ) ;
end

% F_p term of the gravity :	equation (32) / L_PM / J_p

F_p = exp( -omega*(sqrt(u)-1)/sqrt(10) ) ;

% F_m term of the capillarity : equation (41)

F_m = exp( -0.25 * (k/km-1).^2 ) ;

% J_p term of the gravity : equation (3)

if omega > 1
	kapa = 1.7 + 6 * log10(omega) ;
else
	kapa = 1.7 ;
end

sigma = 0.08 * ( 1 + 4/omega^3 ) ;
J_p = kapa .^ exp( -(sqrt(u)-1).^2 / 2 / sigma^2 ) ;

% Pierson-Moskowitz term of the gravity : equation (2)

L_PM = exp( -1.25./u.^2 ) ;

% Full Spectrum : gravity + capillarity : equation (30)

S = alpha_p * cp * F_p .* J_p + alpha_m * cm * F_m ;
S = 0.5 * S .* L_PM ./ c ./ k.^3 ;
...
%
% Fonction angulaire de Elfouhaily symetrisee Equation (57) :
% (Partie anisotrope du spectre de Elfouhaily)
%
%   S = elfo_ang( u10 , k , omega )
%
% Entrees :
%
%   u10     : Vitesse à 10 m en m/s
...
function S = elfo_ang( U10 , k , omega )

% Calcul des parametres

g = 9.81 ; % Intensite de la pesanteur
km = 363 ; % Nombre d onde du pic en rad/m
cm = 0.23 ;	% Vitesse de phase du pic en m/s
cp = U10 / omega ; % Vitesse de l'inverse de l'age de la vague
c  = sqrt(g*(1+k.^2/km^2)./k ) ; % Vitesse de phase en m/s
...
S = tanh( a0 + ap * (c/cp).^2.5 + am .* (cm./c).^2.5 ) ;
