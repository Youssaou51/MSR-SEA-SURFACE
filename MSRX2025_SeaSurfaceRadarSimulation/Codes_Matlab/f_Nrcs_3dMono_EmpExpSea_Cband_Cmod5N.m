function [Nrcs_Mdl_VV, Nrcs_Mdl_HH] = f_Nrcs_3dMono_EmpExpSea_Cband_Cmod5N (Th_i_d, u10_ms, windDir_d) 

%--------------------------------------------------------------------------
% FUNCTION : f_Nrcs_3dMono_EmpExpSea_Cband_Cmod5N
%--------------------------------------------------------------------------
...
% - u10_ms must be between ...-... m/s (1-40???) 
...
% Functions [Hersbach_2007_JGR,eq.A5-A6,A12]:
...
% Calculation of B_0 term [Hersbach_2007_JGR,eq.A2]:
G_S0_ = 1 ./ (1 + exp(-S_0       )) ; % [Hersbach_2007_JGR,eq.A4]
G_A2v = 1 ./ (1 + exp(-A_2*u10_ms)) ; % [Hersbach_2007_JGR,eq.A4]
Al = S_0 .* (1 - G_S0_) ; % [Hersbach_2007_JGR,eq.A4]
% F_A2v_S0 = S_0.^Al .* G_S0_ .* (A_2*u10_ms < S_0) + G_A2v .* (A_2*u10_ms >= S_0) ; % [Hersbach_2007_JGR,eq.A3]
F_A2v_S0 = (A_2.*u10_ms./S_0).^Al .* G_S0_ .* (A_2*u10_ms < S_0) + G_A2v .* (A_2*u10_ms >= S_0) ; % [Hersbach_2003_TechReport,eq.25]
%
B_0 = 10.^(A_0 + A_1*u10_ms) .* F_A2v_S0.^Gmm ; % [Hersbach_2007_JGR,eq.A2]
% B_0_dB = 10*log10(B_0) 

% Calculation of B_1 term [Hersbach_2007_JGR,eq.A7]:
TanhB1 = tanh(4 * (X+c16+c17*u10_ms)) ;
Num_B1 = c14 * (1+X) - c15*u10_ms * (0.5+X-TanhB1) ;
B_1 = Num_B1 ./ (1 + exp(0.34*(u10_ms-c18))) ;

% Calculation of B_2 term [Hersbach_2007_JGR,eq.A8]:
y0 = c19 ; n = c20 ; % [Hersbach_2007_JGR,eq.A10]
a = y0 - (y0-1)/n ; % [Hersbach_2007_JGR,eq.A11]
b = 1 / ( n * (y0-1)^(n-1) ) ; % [Hersbach_2007_JGR,eq.A11]
Y = (u10_ms + Nu0) ./ Nu0 ;
...
Nrcs_Mdl_VV = B_0 .* ( 1 + B_1 * cosd(1*windDir_d) + B_2 * cosd(2*windDir_d) ).^p ; % [Hersbach_2010_JAOT,eq.3] 
