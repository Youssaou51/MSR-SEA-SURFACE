function [ H , X_h , S_K , K , ls_gen , k_min , k_max ] = ...
    f_GeneSurfMer2D_ElfoGauss_Fetch_v2b_Time_v1 ( u10 , l_s , n_s , sd , fetch_m , time_s , prec ) 

%--------------------------------------------------------------------------
% FUNCTION: f_GeneSurfMer2D_ElfoGauss_Fetch_v2b_Time_v1 
%--------------------------------------------------------------------------
...
% NB: This version takes time into account as input, which is useful for 
% computing a time-evolving sea surface 
...
% time_s    : Time instant of simulation (s) [scalar]
...
% Omg = sqrt(g.*K.*(1+(K./km).^2.*tanh(K.*300000))) ; % Pulsation (dispersion relationship) (rad/s) - Goulven MONNIER, code "SpectrumToSurface.m", line 43 
% Omg = sqrt(g*K.*(1+K.^2/km^2))  % Pulsation (dispersion relationship) (rad/s) - cf. Phase speed [1982_Fung_JOE], [1997_Elfouhaily_JGR,eq.24] 
% Omg = sqrt(g*K) ; % Pulsation (dispersion relationship) (rad/s) - cf. Phase speed [1982_Fung_JOE], [1997_Elfouhaily_JGR,eq.24] 
% Exp_iwt = exp( 1i .* Omg * time_s ) ; % Phase shift due to time evolution [Tessendorf_2004_CourseNotes,eq.43] - https://jtessen.people.clemson.edu/reports/papers_files/coursenotes2004.pdf 
% Exp_iwt = conj(Exp_iwt) ; 
Kpos = abs(K) ; 
Omg = sqrt(g*Kpos.*(1+Kpos.^2/km^2)) ; % Pulsation (dispersion relationship) (rad/s) - cf. Phase speed [1982_Fung_JOE], [1997_Elfouhaily_JGR,eq.24] 
Exp_iwt = exp( -sign(K) .* 1i .* Omg * time_s ) ; % Phase shift due to time evolution [Tessendorf_2004_CourseNotes,eq.43] - https://jtessen.people.clemson.edu/reports/papers_files/coursenotes2004.pdf 
Sz = Sz .* Exp_iwt ; % Modification of Sz to account for time evolution 
