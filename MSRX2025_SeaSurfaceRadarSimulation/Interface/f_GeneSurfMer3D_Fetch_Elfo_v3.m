function [ HH_XY , X , Y , SP_KXKY , Kx , Ky , NOr_XY ] = ...
    f_GeneSurfMer3D_Fetch_Elfo_v3 ( u10 , phiw_d , l_x , l_y , n_x , n_y , sd , prec , fetch_m ) 

%--------------------------------------------------------------------------
% FUNCTION: f_GeneSurfMer3D_Fetch_Elfo_v3
%--------------------------------------------------------------------------
...
% elfo_ang (below)
%--------------------------------------------------------------------------
% Nicolas PINEL - PhD, 11/2013
% Version: 12/2025 
%--------------------------------------------------------------------------

%% Profilage mémoire 

% Initialisation du log mémoire global

fprintf('\n Début de génération surface %d×%d (prec=%d) \n', n_x, n_y, prec);
logMemory('Début');
...
COs2Phi = (KKx.^2-KKy.^2)./KK.^2 * cos(2*phiw_d*pi/180) + 2*KKx.*KKy./KK.^2 * sin(2*phiw_d*pi/180) ; 
logMemory('COs2PhiObs');
clear KKx KKy 
...
%
SP_KXKY = MM_KXKY .* (1+COs2Phi.*DD_KXKY)./(2*pi) ; 
SP_KXKY = SP_KXKY ./ KK ; % ATTENTION JACOBIEN TRANSFORMATION... 1/K 
logMemory('SP_KXKY');
...
% spx_Elfo0 = sqrt( trapz(Ky,trapz(Kx,SP_KXKY./1.*KKx.^2)) ) ; % Voir These Christophe, eq. I.85
% spx_Elfo1 = sqrt( trapz(Ky,trapz(Kx,MM_KXKY  .* (1+COs2Phi.*DD_KXKY )./(2*pi).*KKx.^2./KK)) ) ; % Voir These Christophe, eq. I.85
...
% spy_Elfo0 = sqrt( trapz(Ky,trapz(Kx,SP_KXKY./1.*KKy.^2)) ) ; % Voir These Christophe, eq. I.85
% spy_Elfo1 = sqrt( trapz(Ky,trapz(Kx,MM_KXKY  .* (1+COs2Phi.*DD_KXKY )./(2*pi).*KKy.^2./KK)) ) ; % Voir These Christophe, eq. I.85
...
% clear COsCarrePhi SInCarrePhi 
clear COs2Phi DD_KXKY MM_KXKY KK %KKx KKy 
logMemory('SP_KXKY + nettoyage');
...
fprintf('Hermitian GWN (Gaussian white noise) generation... ') ; 
...
% for ix = 2 : 1 : n_x
%     for iy = 2 : 1 : n_y
%         BH1(ix,iy) = 0.5 * ( B0(ix,iy) + conj(B0(n_x+2-ix,n_y+2-iy)) ) ;
%     end
% end
% toc
% 'A OPTIMISER'
...
Ix = 2:n_x;
Iy = 2:n_y;
% Utilise l'indexation matricielle implicite (broadcasting via bsxfun ou opérations matricielles)
BH(Ix,Iy) = 0.5 * ( B0(Ix,Iy) + conj( B0(n_x+2-Ix, n_y+2-Iy) ) );
% find(BH-BH1)
% toc
%
clear B0
logMemory('Bruit hermitien BH');
% whos
% pause
fprintf('OK\n') ; 
...
fprintf('IFFT2 normalisation constant... ') ;
...
NOr_XY = (Nor_X(:) * Nor_Y(:).') * n_x * n_y * sqrt( pask_x * pask_y ) ; % Normalisation imposee par Matlab
fprintf('OK\n') ; 
logMemory('NOr_XY');
...
fprintf('Calculation of the IFFT2... ') ;
...
%% Fonction pour le profilage
function logMemory(stepName)
        v = evalin('caller', 'whos');
        totalMo = sum([v.bytes]) / 1024^2;
        fprintf(' [MEM] %-40s  %.1f Mo\n', stepName, totalMo);
end