function Sig0_HYB_dB = f_Nrcs_3dMonoGrazing_HYB2 (Chi_d, lambda_m, seaState, ph__d, polar)
...
for ii = 1 : length(Chi_d)
    if cht_d >= chr_d
        if Chi_d(ii) < chr_d
            Kg(ii) = 0 ;
        elseif Chi_d(ii) <= cht_d
            Kg(ii) = 20*log10(Chi_d(ii)/chr_d) ;
        elseif Chi_d(ii) <= 30
            Kg(ii) = 20*log10(cht_d/chr_d) + 10*log10(Chi_d(ii)/cht_d) ;
        else
            error('ATTENTION : Angle (= %.1f°) pas pris en compte dans le modèle (doit être <= 30°)', Chi_d(ii)) ;
            return ;
