...
l_s = 32 ; % Desired length of generated surface (m) 
...
% 
timeStep = 0.1 ; % Time interval between two successive surfaces (s) 
% timeStep = 0.2 ; % Time interval between two successive surfaces (s) 
n_tmSt = 200 ; % Number of time steps (number of realizations of the same time-evolving surface) (-) 
% n_tmSt = 5 ; % Number of time steps (number of realizations of the same time-evolving surface) (-) 
% https://www.youtube.com/watch?v=z79sYv41cVI
...
% for i_prof = 1 : n_prof 
    ...
    % [ HH(i_prof,:) , X_h , S_K , K , ls_gen , k_min , k_max ] = f_GeneSurfMer2D_ElfoGauss_Fetch_v2b ( u10 , l_s , n_s , sd , fetch_m , time_s , prec ) ; 
% end
sd = 1 ; % Seed of the generated white Gaussian noise (-) 
for i_tmSt = 1 : n_tmSt 
    time_s = (i_tmSt-1) * timeStep ; 
    [ HH(i_tmSt,:) , X_h , S_K , K , ls_gen , k_min , k_max ] = f_GeneSurfMer2D_ElfoGauss_Fetch_v2b_Time_v1 ( u10 , l_s , n_s , sd , fetch_m , time_s , prec ) ; 
end
% end
...
for i_tmSt = 1 : n_tmSt 
    ...
    [ Psi_inc , i_inc ] = F_IIncidenteWave_Thorsos( th_i_d , g, k0_EM, X_h, HH(i_tmSt,:) ) ; 
    %
    ...
    DerZ = gradient(HH(i_tmSt,:)) ./ Dx ; % First derivative of H over X_h (-) --- DerZ = diff(Z) ; DerZ = [ DerZ(1) DerZ ]/dx ; 
    ...
    tic
    ZZ = F_IImpedanceMatrices( X_h , HH(i_tmSt,:) , Dx , DerZ , Der2Z , V , k0_EM , Polar ) ;
    dt_ZZ = toc 
    %
    ...
    Psi_sca = F_IFarField( X_h, HH(i_tmSt,:), Dx , DerZ , V , k0_EM , Psi , D_Psi , Th_s_r ) ;
