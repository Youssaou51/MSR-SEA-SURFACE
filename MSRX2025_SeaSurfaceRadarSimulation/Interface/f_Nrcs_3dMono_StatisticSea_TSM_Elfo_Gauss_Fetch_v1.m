function [Nrcs_Mdl_VV, Nrcs_Mdl_HH, Nrcs_Mdl_VH, sig_sx_LS, sig_sy_LS, AddTerm_VV, AddTerm_HH] = ...
    f_Nrcs_3dMono_StatisticSea_TSM_Elfo_Gauss_Fetch_v1 (Th_i_d, phi_rel_d, er2, u_10, fetch_m, k0_EM, kc) 

%--------------------------------------------------------------------------
% FUNCTION : f_Nrcs_3dMono_StatisticSea_TSM_Elfo_Gauss_Fetch_v1 
%--------------------------------------------------------------------------
...
n_tp = 100 ; % Number of sampling points for integration over tan(psi_loc  ) (-) 
n_td = 100 ; % Number of sampling points for integration over tan(delta_loc) (-) 
...
% sig_sx_int = sig_sx_LS ;
% sig_sy_int = sig_sy_LS ;
...
for i_th = 1 : length(Th_i_d)
    ...
    %
    Nrcs_Mdl_HH(i_th) = trapz( TanPsi_l , trapz( TanDlt_l , SIg0_LocSPM_HH .* PSloc_Gss2D_LS ) ) ;
    Nrcs_Mdl_VV(i_th) = trapz( TanPsi_l , trapz( TanDlt_l , SIg0_LocSPM_VV .* PSloc_Gss2D_LS ) ) ;
    Nrcs_Mdl_VH(i_th) = trapz( TanPsi_l , trapz( TanDlt_l , SIg0_LocSPM_VH .* PSloc_Gss2D_LS ) ) ;
