        ...
        for k = 1:n_seastates
            for m = 1:n_pol
                for p = 1:n_ph
                    ...
                    nrcs_toolbox = refl(GrazAng_d(i), freq_Hz, ph__d(p)); % 3 arguments
...
% MAE globale
deltas = abs(results(:, :, :, :, :, 2) - results(:, :, :, :, :, 1)); % Écarts absolus sur toutes les combinaisons
deltas(isinf(deltas) | isnan(deltas)) = 0; % Remplace Inf/NaN par 0
mae = mean(deltas, 'all', 'omitnan'); % Moyenne globale sur toutes les dimensions
...
for i = 1:n_angles
    ...
    plot(freq_GHz, squeeze(results(i, :, idx_seast, idx_pol, 1, :)), '-o', 'LineWidth', 1.5); % ph__d=1 (0°), 1 freq
