...
% MAE globale
deltas = abs(results(:, :, :, :, 2) - results(:, :, :, :, 1)); % Écarts absolus
deltas(isinf(deltas) | isnan(deltas)) = 0; % Remplace Inf/NaN par 0 pour éviter de fausser la moyenne
...
for i = 1:n_angles
    ...
    plot(freq_GHz, squeeze(results(i, :, idx_seast, idx_pol, :)), '-o', 'LineWidth', 1.5);
