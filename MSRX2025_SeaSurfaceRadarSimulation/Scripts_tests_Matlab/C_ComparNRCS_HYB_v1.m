...
% MAE globale (moyenne sur toutes les directions de vent)
deltas = abs(results(:, :, :, :, :, 2) - results(:, :, :, :, :, 1));
deltas(isinf(deltas) | isnan(deltas)) = 0; 
mae = mean(deltas, 'all', 'omitnan');
disp(['MAE global (dB) : ' num2str(mae)]);

% Affichage réduit : Seulement pour pol='V', SeaState=5, et toutes les directions de vent
idx_pol = 2; % 'V'
idx_seast = 5; % SeaState = 5 (index 6 pour 0:5)
disp('Comparaison NRCS (dB) pour pol=V, SeaState=5 :');
disp('Angle | Freq (GHz) | ph__d (deg) | Toolbox | Ma version | Δ');
for i = 1:n_angles
    for j = 1:n_freqs
        for p = 1:n_ph
            toolbox_val = results(i, j, idx_seast, idx_pol, p, 1);
            my_val = results(i, j, idx_seast, idx_pol, p, 2);
            if ~isnan(toolbox_val)
                fprintf('%4.1f° | %6.2f | %9.0f | %8.2f | %8.2f | %8.2f\n', ...
                    GrazAng_d(i), freq_GHz(j), ph__d(p), toolbox_val, my_val, my_val - toolbox_val);
            end
        end
    end
end
...
for i = 1:n_angles
    for p = 1:n_ph
        ...
        plot(freq_GHz, squeeze(results(i, :, idx_seast, idx_pol, p, :)), '-o', 'LineWidth', 1.5);
